...
if($db){
    try{
        ...
        $query = $db->query("SELECT * FROM `utilisateurs` INNER JOIN `particulier` ON particulier.id_utilisateur = utilisateurs.id WHERE utilisateurs.id = '$id_utilisateur'");
        ...
        $error = $e->getMessage();
        $logger->error("Echec de la Creation d'une annonce (particulier) -- $error");
    }
}else{
